...
class test(QObject):
    realSignal = pyqtSignal(dict)

class Kiwoom(QAxWidget):
#    realSignal = pyqtSignal(list)
    ...
    def eventConnect(self, returnCode):
        ...
        try:
            if returnCode == ReturnCode.OP_ERR_NONE:
                ...
                self.codeList = self.getCodeList("0")
            else:
    ...
    def receiveTrData(self, screenNo, requestName, trCode, recordName, inquiry,
        ...
        """
    rtJusikChaekulLists = ['체결가','최우선매도호가','최우선매수호가','누적체결량','누적거래대금','시가','고가','저가']   # 10;27;28;13;14;15;16;17;18

    def getCodeNameFromCode(self, code):
        codeName = ""
        if code in self.codeList:
            codeName = self.getMasterCodeName(code)
        return codeName

    def setFidFromName(self, realType, field):
        for fid in RealType.REALTYPE[realType].keys():
            if (RealType.REALTYPE[realType][fid] == field):
                return fid
        return -1
...
#            self.log.debug("({})".format(realType))
            ...
            if code != "":
                ...
                codeOrNot = realType
            """
            realData['RealType'] = realType

            if (realType == '주식체결'):
                data["종목코드"] = code
                for field in self.rtJusikChaekulLists:
                    fid = self.setFidFromName(realType, field)
                    if (fid == -1):
                        print("RealType field error:", realType, field, fid)
                        continue
                    value = self.getCommRealData(realType, fid)
                    data[field] = value
                realData['Data'] = data
            else:
                data["종목코드"] = code
                for fid in RealType.REALTYPE[realType].keys():
                    value = self.getCommRealData(realType, fid)
                    data[RealType.REALTYPE[realType][fid]] = value
            ...
            self.r.realSignal.emit(realData)
    ...
    def makeRealRegDataList(self, realType):
        fidListStr = ""
        for fieldname in self.rtJusikChaekulLists:
            for fid in RealType.REALTYPE[realType].keys():
                if (RealType.REALTYPE[realType][fid] == fieldname):
                    fidListStr += str(fid) + ';'
                    break

        return fidListStr

    def set_real_req(self, code):
        realDatas = self.makeRealRegDataList("주식체결")
        if (realDatas != ""):
            self.setRealReg("0101", code, realDatas, "0")
